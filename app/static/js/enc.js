...
async function aesEncrypt(data, salt){
    const keyIv = evpBytesToKey(salt, PASSWORD);
    const cryptoKey = await crypto.subtle.importKey("raw", keyIv.key, "AES-CBC", false, ["encrypt"]);
    const encrypted = await crypto.subtle.encrypt({
        name: "AES-CBC",
        iv: keyIv.iv
    }, cryptoKey, data);
    return {
        ct: new Uint8Array(encrypted).toBase64(),
        iv: keyIv.iv.toHex(),
        s: salt.toHex()
    };
}
...
async function encodeEncdata(data) {
    const textData = JSON.stringify(data);
    const binData = new TextEncoder().encode(textData);
    let salt = new Uint8Array(8);
    crypto.getRandomValues(salt);
    return await aesEncrypt(binData, salt);
}
